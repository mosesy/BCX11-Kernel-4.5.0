...
#define RELEASE_DATE "2016/09/01"
...
static int __devinit egalax_i2c_probe(struct i2c_client *client, const struct i2c_device_id *idp)
{
	...
	client->irq = gpio_to_irq(EXC31XX_GPIO_INT);
#ifdef CONFIG_OF
...
#endif //#ifdef CONFIG_OF
	p_egalax_i2c_dev->interrupt_gpio = EXC31XX_GPIO_INT;
...
#ifdef CONFIG_ACPI
static struct acpi_device_id eeti3062_acpi_match[] = {
	{"EETI3062", 0},
	{},
};

MODULE_DEVICE_TABLE(acpi, eeti3062_acpi_match);
#endif
...
static struct i2c_driver egalax_i2c_driver = {
	.driver = {
		...
		.owner	= THIS_MODULE,
#ifdef CONFIG_ACPI
		.acpi_match_table = ACPI_PTR(eeti3062_acpi_match),
#endif
	#ifdef CONFIG_OF
