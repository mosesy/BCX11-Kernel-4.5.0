...
static int byt_hs_detection(void)
{
	...
	if (!status) {
		...
		jack_type = SND_JACK_HEADPHONE;
		byt_force_enable_pin(codec, "Headphone", true);
		byt_force_enable_pin(codec, "Ext Spk", false);
	} else {
		pr_debug("Jack out");
		rt5645_headset_detect(codec, false);
		jack_type = 0;
		byt_force_enable_pin(codec, "Headphone", false);
		byt_force_enable_pin(codec, "Ext Spk", true);
	}
	byt_force_enable_pin(codec, "Int Mic", true);
