...
int sst_send_probe_bytes(struct intel_sst_drv *sst)
{
	...
	printk("%s 3 msg=%p msg->header=%p\n", __func__, msg, &msg->header);
	printk("%s 3 sst->probe_bytes->len=%d,\n", __func__, sst->probe_bytes->len);
	msg->header.part.data = sizeof(u32) + sst->probe_bytes->len;
	printk("%s 4\n", __func__);
	printk("%s 4 msg->header.full=%p\n", __func__, &msg->header.full);
	printk("%s 4 msg->mailbox_data=%p\n", __func__, msg->mailbox_data);
	printk("%s 4 msg->header.part.data=%d\n", __func__, msg->header.part.data);
	memcpy(msg->mailbox_data, &msg->header.full, sizeof(u32));
	printk("%s 5 sst->probe_bytes->bytes=%p sst->probe_bytes->len=%d\n", __func__, sst->probe_bytes->bytes, sst->probe_bytes->len);	
